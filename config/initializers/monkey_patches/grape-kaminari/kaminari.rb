          ...
          private
            ...
            def wrap(collection)
              if collection.is_a?(ActiveRecord::Relation)
                collection.page(params[:page].to_i).per(params[:per_page].to_i).padding(params[:offset].to_i)
              elsif collection.is_a?(Array)
                ::Kaminari.paginate_array(collection).page(params[:page].to_i).per(params[:per_page].to_i).padding(params[:offset].to_i)
              else
                raise "#{collection.class}クラスのcollectionはサポートしていません"
              end
            end
        end
